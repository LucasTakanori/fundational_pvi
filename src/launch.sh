...
# Load on compute nodes (versions may differ from the login node).
# module load CUDA/12.9.0
# module load gcc/11.2.0
...
# Cluster paths (artifacts + masked HDF5 under …/pvi_data/main/).
# shellcheck source=/dev/null
source "${REPO_ROOT}/env/cluster.env"

# uv-managed virtual environment (create once: uv sync)
...
# Override to run a different entry point, e.g. pretrain:
#   export JOB_SCRIPT="python -u -m src.foundation.pretrain ..."
# For transfer, use src/launch_transfer.sh instead (unsets PVI_CACHE_ROOT).
JOB_SCRIPT="${JOB_SCRIPT:-python -u -m src.foundation.pretrain --input-mode signal --output-mode waveform --max-epochs 500 --batch-size 512 --cache-num-workers 8}"
