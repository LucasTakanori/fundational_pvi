...
class ProjectRoot:
    def __init__(self, rootdir: str|Path=None) -> None:
        # Resolution order (OS-independent):
        #   1. explicit `rootdir` argument
        #   2. the PVIPROJECT_ROOT environment variable
        #   3. a repo-relative ./data directory (so the package runs out-of-the-box)
        if rootdir is not None:
            self.root = Path(rootdir)
        elif "PVIPROJECT_ROOT" in os.environ:
            self.root = Path(os.environ["PVIPROJECT_ROOT"])
        else:
            # primitives.py lives at <repo>/src/utils/ -> parents[2] is <repo>
